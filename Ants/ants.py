...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place != hive:
            if self.min_range <= in_range <= self.max_range:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if len(bee_list) > 0:
            if self.digesting == 0 and len(bee_list) > 0:
                self.eat_bee(random_or_none(bee_list))
    ...
    def action(self, colony):
        ...
        if self.place is not None:
            if self.impostor:
                ...
                tunnel_loc = self.place
                while tunnel_loc is not None:
                    self.power_up(tunnel_loc)
                    tunnel_loc = tunnel_loc.entrance
                tunnel_loc = self.place.exit
                while tunnel_loc is not None:
                    self.power_up(tunnel_loc)
                    tunnel_loc = tunnel_loc.exit

    def power_up(self, tunnel):
        if tunnel.ant is not None and tunnel.ant != self \
        and tunnel.ant not in self.powered_up:
            tunnel.ant.damage *= 2
            self.powered_up.append(tunnel.ant)
            if isinstance(tunnel.ant, BodyguardAnt) and tunnel.ant.ant \
            and tunnel.ant.ant != self and tunnel.ant.ant not in self.powered_up:
                tunnel.ant.ant.damage *= 2
                self.powered_up.append(tunnel.ant.ant)
