...
class SimplePageRank(object):
    ...
    def update_weights(nodes, num_nodes):
        ...
        You are allowed to change the signature if you desire to.
        """
        # nodes = the list containing all the nodes?
        # num_nodes = the total number of nodes, 3 from example

        def distribute_weights((node, (weight, targets))):
            # YOUR CODE HERE

            node_lst = []
            node_weight = weight * 0.05  # 5% of the time we stay on same page 
            node_lst.append((node, node_weight))

            if len(targets) == 0: #if the node has no outgoing link
                for n in range(num_nodes): #go through each of the other nodes and give them weight 
                    if n == node: #if its the same node we dont do anything
                        continue
                    w = (weight * 0.85) / (num_nodes - 1)
                    node_lst.append((n, w))
            else:
                for t in targets: #go through each target and give them weight
                    target_weight = weight * (0.85 / len(targets))
                    node_lst.append((t, target_weight))
            
            node_lst.append((node, targets)) #special tuple to remember targets for the node 
            return node_lst    
        ...
        def collect_weights((node, values)):
            # YOUR CODE HERE
            # dampening factor is just 0.10 because
            #   (0.10 * sum_of_all_weights) / (num_nodes) = 0.10
            # since sum_of_all_weights / num_nodes is always 1

            target_lst = []
            node_weight = 0.10 #node weight is the dampening factor
            ...
            for v in values:
                if isinstance(v, frozenset):
                    ...
                    node_weight += v
            return (node, (node_weight, target_lst)) # return the updated weight for NODE
