...
def eval_all(expressions, env):
    ...
    while index < len(expressions):
        if index == len(expressions) - 1:
            return scheme_eval(expressions[index], env, True)
        else:
            scheme_eval(expressions[index], env)
        index += 1
...
def make_call_frame(procedure, args, env):
    ...
    if isinstance(procedure, MuProcedure):
        if len(args) > 0:
            env.bindings[procedure.formals.first] = args.first
        return Frame(env)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError("the number of formal parameters and values does not match")
        for formal, values in zip(formals, vals):
            frame.bindings[formal] = values
        return frame
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        lambda_exprs = Pair(expressions.first.second, expressions.second)
        env.define(target.first, do_lambda_form(lambda_exprs, env))
        return target.first
    else:
...
def do_if_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    first_expr = scheme_eval(expressions[0], env)
    if scheme_true(first_expr):
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 0:
        ...
        while index < len(expressions):
            if index == len(expressions) - 1:
                return scheme_eval(expressions[index], env, True)
            expr = scheme_eval(expressions[index], env)
...
def do_or_form(expressions, env):
    ...
    if len(expressions) == 0:
        ...
        index = 0
        while index < len(expressions):
            expr = scheme_eval(expressions[index], env, True)
            if scheme_true(expr):
                return expr
            index += 1
        return False
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    child_frame = Frame(env)
    for binding in bindings:
        if scheme_numberp(binding[0]):
            raise SchemeError
        check_form(binding, 2, 2)
        child_frame.define(binding[0], scheme_eval(binding[1], env))
    return child_frame
...
def check_formals(formals):
    ...
    for formal in formals:
        if formal in symbol_list or scheme_symbolp(formal) == False:
            raise SchemeError("invalid list of formals")
        symbol_list.append(formal)
